...
def Ckuser(message):
  ...
  response = requests.get('https://api.telegram.org/bot{}/getchatmember?chat_id=@{}&user_id={}'.format(TOKEN,uui9u,userID)).json()
  if response["ok"] == False:
    return True
  if response["result"] != None and response["result"]["status"] == "left":
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("اضغط للاشتراك ⏺", url="t.me/"+uui9u)] ])
    Bot("sendMessage",{"chat_id":chatID,"text":'عذراً يجب عليك الاشتراك في <a href="t.me/'+uui9u+'">قناة البوت</a>\nليمكنك استخدام البوت بشكل كامل',"reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True,"reply_markup":kb})
    return False
  else:
    return True
