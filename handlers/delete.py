...
def delete(client, message,redis):
  ...
  if text :
    ...
    if message.forward_date:
        if (rank is False or rank is 0) and group is True and redis.sismember("{}Nbot:Lfwd".format(BOT_ID),chatID):
            Bot("deleteMessage",{"chat_id":chatID,"message_id":message.message_id})
        if not (rank is "sudo" or rank is "asudo" or rank is "sudos") and group is True and redis.sismember("{}Nbot:Lfwd".format(BOT_ID),chatID) and not message.text:
            Bot("deleteMessage",{"chat_id":chatID,"message_id":message.message_id})
        if not (rank is "sudo" or rank is "asudo" or rank is "sudos") and group is True and not redis.sismember("{}Nbot:Lfwd".format(BOT_ID),chatID):
