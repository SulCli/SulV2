...
def updateCallback(client, callback_query,redis):
  ...
  if group is True and int(date[2]) == userID and not redis.get("{}Nbot:floodUsers:{}".format(BOT_ID,userID)):
    ...
    if date[0] == "dlf":
      File = date[1]
      os.system("rm ./files/"+File)
      url = "https://raw.githubusercontent.com/SulCli/SulV2-files/master/"+File
    ...
    if date[0] == "ShowDateUser":
      ...
      age = getAge(userID,r)
      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(Name(userFN),url="t.me/"+uui9u)],[InlineKeyboardButton(r.Rrank.format(t),url="t.me/"+uui9u)],[InlineKeyboardButton(r.Rmsgs.format(msgs),url="t.me/"+uui9u)],[InlineKeyboardButton(r.Rrate.format(str(rate)+"%"),url="t.me/"+uui9u)],[InlineKeyboardButton(r.Redits.format(edits),url="t.me/"+uui9u)],[InlineKeyboardButton(r.Rage.format(age),url="t.me/"+uui9u)]])
