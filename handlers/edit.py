...
def edit(client, message,redis):
    ...
    if not message.outgoing:
        ...
        if not (rank is "sudo" or rank is "asudo" or rank is "sudos") and group is True and redis.sismember("{}Nbot:Ledits".format(BOT_ID),chatID) and not message.text:
            Bot("deleteMessage",{"chat_id":chatID,"message_id":message.message_id})
        if not (rank is "sudo" or rank is "asudo" or rank is "sudos") and group is True and not redis.sismember("{}Nbot:Ledits".format(BOT_ID),chatID):
